<template>
    <div class="card h-100 w-100" style="auto">
        ...
        <div class="card-footer text-body-secondary">
            <a href="#head" type="button" class="btn btn-danger" :click="deleteSession">
    ...
    methods: {
        ...
        async deleteSession() {
            if (!this.doubleConfirm) {
                ...
                alert("If you know what you are doing right now, click the trash bin button once again.")
                this.doubleConfirm++;
            } else {
                var response = await fetch(`${this.$root.apiUrl}/api/session/${session.id}`, {
                    method: 'DELETE'
                });
