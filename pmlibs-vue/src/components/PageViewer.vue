<template>
    <div class="container">
        <div class="container-fluid text-center mt-5 mb-5 pt-4 pb-4 bg-light-subtle border border-light-subtle rounded-3">
            <div class="row">
        ...
        <Transition>
            <session-viewer :session="retreiveCurrentSession()" v-if="this.$root.activePage == this.$root.appPage.SESSION"></session-viewer>
        </Transition>
        <Transition>
            <bookshelf-group-viewer :session="retreiveCurrentSession()" v-if="this.$root.activePage == this.$root.appPage.GROUP"></bookshelf-group-viewer>
        </Transition>
        <Transition>
            <bookshelf-viewer :session="retreiveCurrentSession()" v-if="this.$root.activePage == this.$root.appPage.BOOKSHELF"></bookshelf-viewer>
        </Transition>
    </div>
